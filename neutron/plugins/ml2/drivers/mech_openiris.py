...
iris_opts = [
    cfg.StrOpt('url',
               help=_("HTTP URL of OpenIRIS REST interface.")),
    cfg.StrOpt('username',
...
cfg.CONF.register_opts(iris_opts, "ml2_openiris")
...
class JsessionId(requests.auth.AuthBase):
    ...
    def __init__(self, url, username, password):
        ...
        self.expired = None
        self.session_timeout = cfg.CONF.ml2_openiris.session_timeout
...
class OpenIRISMechanismDriver(api.MechanismDriver):

    """Mechanism Driver for OpenIRIS.

    This driver was a port from the Tail-F NCS MechanismDriver.  The API
    exposed by OpenIRIS is slightly different from the API exposed by NCS,
    ...
    def initialize(self):
        self.url = cfg.CONF.ml2_openiris.url
        self.timeout = cfg.CONF.ml2_openiris.timeout
        self.username = cfg.CONF.ml2_openiris.username
        self.password = cfg.CONF.ml2_openiris.password
    ...
    def synchronize(self, operation, object_type, context):
        """Synchronize OpenIRIS with Neutron following a configuration change."""
        if self.out_of_sync:
    ...
    def sync_full(self, context):
        """Resync the entire database to OpenIRIS.
        ...
        for port in ports:
            try:
                ...
                if e.response.status_code == 404:
                    ...
                    # TODO(kmestery): Converting to uppercase due to OpenIRIS bug
    ...
    def sync_object(self, operation, object_type, context):
        """Synchronize the single modified record to OpenIRIS.
        """
        ...
        if operation == 'delete':
            ...
            if object_type == 'networks':
                ...
                try:
                    ...
                    if operation == 'create':
                        # TODO(kmestery): Converting due to OpenIRIS bug
                        port['mac_address'] = port['mac_address'].upper()
                        if 'status' in port:
                            try:
                                del port['status']
    ...
    def sendjson(self, method, urlpath, obj):
        ...
        if self.url:
            ...
            LOG.debug(_('OpenIRIS-----> sending URL (%s) <-----OpenIRIS') % url)
            LOG.debug(_('OpenIRIS-----> sending JSON (%s) <-----OpenIRIS') % obj)
    ...
    def check_segment(self, segment):
        """Verify a segment is valid for the OpenIRIS MechanismDriver

        Verify the requested segment is supported by OpenIRIS and return True or
